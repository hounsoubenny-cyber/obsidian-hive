...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug  2 02:49:42 2026
...
class LLMManager:
    ...
    async def execute_tool(
        ...
        await self._fire_event("tool_exec_before", name, args)

        if name not in tool_map:
            result = f"❌ Outil inconnu : {name}"
            await self._exec_callback(on_error, name, args, ValueError(result))
            await self._fire_event("tool_exec_error", name, args, ValueError(result))
            return result
        ...
        try:
            ...
            await self._fire_event("tool_exec_error", name, args, e)
            return result

        await self._exec_callback(on_after, name, args, result)
        await self._fire_event("tool_exec_after", name, args, result)
        ...
        while iteration < max_iter:
            ...
            if choice.finish_reason == "stop" or not choice.message.tool_calls:
                ...
                for tool_call in choice.message.tool_calls:
                    ...
                    tool_calls[call_key]["ended_at"] = time.time()
                    try:
                        tool_args = json.loads(tool_call.function.arguments)
                        if not isinstance(tool_args, dict):
                            tool_args = {}
                        tool_calls[call_key]["args"] = tool_args
                    except json.JSONDecodeError as e:
                        tool_result = f"❌ Arguments invalides pour {tool_name}: {e}"
                        tool_calls[call_key]["success"] = False
                        tool_calls[call_key]["result"] = tool_result
                        tool_calls[call_key]["ended_at"] = time.time()
                        tool_calls[call_key]["duration"] = tool_calls[call_key]["ended_at"] - tool_calls[call_key]["started_at"]
                    ...
                    tool_calls[call_key]["success"] = True
                    tool_calls[call_key]["result"] = tool_result
                    tool_calls[call_key]["ended_at"] = time.time()
                    tool_calls[call_key]["duration"] = tool_calls[call_key]["ended_at"] - tool_calls[call_key]["started_at"]
