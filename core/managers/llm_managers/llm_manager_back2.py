...
]  # (nom_du_tool, args) — juste avant l'exécution du tool

OnAfterTool = Union[
    Callable[[str, dict, str], None],
    Callable[[str, dict, str], Awaitable[None]]
]  # (nom_du_tool, args, résultat) — juste après l'exécution réussie du tool

OnToolError = Union[
    Callable[[str, dict, Exception], None],
    Callable[[str, dict, Exception], Awaitable[None]]
]  # (nom_du_tool, args, exception) — le tool a levé une erreur (ou nom inconnu)
...
# Même convention que les callbacks existants (sync ou async acceptés),
# mais dédiés aux évènements qui n'existent QUE pendant un stream.
# =============================================================================
OnStreamStart = Union[
...
class StreamEventBus:
    """
    🔔 "Callback des callbacks" pour le streaming.

    Contrairement aux on_step/on_finish/... (un seul callback par appel),
    ce bus permet à PLUSIEURS abonnés d'écouter le même évènement en
    permanence, sans avoir à repasser les callbacks à chaque `.chat()`.
    Utile quand plusieurs agents (Alex, Coralie, ...) ou plusieurs
    consommateurs (ex: websocket UI + logger) partagent le même LLMManager.

    Évènements émis par LLMManager pendant un stream :
        "start"             -> (iteration, model_name)
        "token"              -> (texte_delta, iteration)
        "reasoning_token"    -> (reasoning_delta, iteration)
        "tool_call_delta"    -> (delta_dict, iteration)
        "message"            -> (message_dict complet, iteration)
    """
    ...
    def on(self, event: str, callback: Callable) -> Callable:
        """S'abonne à un évènement. Retourne le callback (utilisable en décorateur)."""
    ...
    async def emit(self, event: str, *args, **kwargs) -> None:
        for callback in list(self._listeners.get(event, [])):
            try:
                ...
                logger.warning(f"[StreamEventBus:{event}] Erreur callback: {e}")
...
class LLMManager:
    def __init__(
        ...
        self.stream_events = StreamEventBus()
    ...
    def on_stream(self, event: str, callback: Callable) -> Callable:
        """Abonnement PERMANENT à un évènement de streaming (voir StreamEventBus).
        Ex: mgr.on_stream("token", lambda t, it: websocket.send(t))"""
        return self.stream_events.on(event, callback)

    def off_stream(self, event: str, callback: Callable) -> None:
        self.stream_events.off(event, callback)

    async def _fire_stream_event(self, event: str, *args, **kwargs) -> None:
        await self.stream_events.emit(event, *args, **kwargs)
    ...
    def _normalize_stream_chunk(self, raw_chunk: Any, provider_family: str) -> List[dict]:
        ...
        if provider_family == "anthropic":
            ...
            if etype == "content_block_start":
                ...
                if stop_reason:
                    events.append({"type": "stop", "finish_reason": stop_reason})
    ...
    async def _stream_completion(
        ...
        await self._fire_stream_event("start", iteration, model_name)
        ...
        async def _handle_event(ev: dict) -> None:
            ...
            if etype == "text_delta":
                ...
                await self._fire_stream_event("token", ev["text"], iteration)
                await self._exec_callback(on_stream_token, ev["text"], iteration)

            elif etype == "reasoning_delta":
                reasoning_parts.append(ev["text"])
                await self._fire_stream_event("reasoning_token", ev["text"], iteration)
                ...
                await self._fire_stream_event("tool_call_delta", ev, iteration)
        ...
        await self._fire_stream_event("message", message.model_dump(), iteration)
            ...
            for attempt in range(max_retries + 1):
                ...
                try:
                    ...
                    if stream:
                        ...
                        if response is not None and provider_family == "anthropic":
                            # Vraie réponse Anthropic (`.content` + `.stop_reason`,
                            # pas de `.choices`) -> on la ramène à la même forme
