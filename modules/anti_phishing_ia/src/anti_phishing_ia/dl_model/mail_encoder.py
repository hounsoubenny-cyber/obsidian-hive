...
# -*- coding: utf-8 -*-
"""
Encodeur BERT pour emails avec contrastive learning.
Peut retourner 2D (pooled) ou 3D (séquentiel).
...
from anti_phishing_ia.phishing_utils.signal_manager import signal_manager
...
class MailEncoder(nn.Module):
    ...
    def build(
        ...
        num_layer_to_freeze = min(max(num_layer_to_freeze, half), num_layers - 1)
        
        # Geler les premières couches
        for layer in getattr(self.model, encoder_layer_name).layer[:num_layer_to_freeze]:
    ...
    def cache(self, tokenizer_output: dict, embedding: torch.Tensor, output2d: bool = False):
        ...
        cache[key] = embedding.cpu().tolist()
    ...
    def load(self, path: str):
        if os.path.exists(path):
            try:
                ...
                self.num_params = sum(p.numel() for p in self.parameters())
                self.to(DEVICE)
                logger.print("Modèle chargé avec succès")
                return
            except Exception as e:
...
class MailEncoderDataset(torch.utils.data.Dataset):
    """Dataset pour l'entraînement contrastif de MailEncoder."""
    ...
    def __init__(
        ...
        if isinstance(text_dataset_path_or_df, pd.DataFrame):
            ...
            self.text_dataset = pd.DataFrame(self._load_data(text_dataset_path_or_df))
        
        assert "text" in self.text_dataset and "label" in self.text_dataset, "Colonnes 'text' et 'label' requises"
    ...
    def __getitem__(self, index):
        ...
        ids = output["input_ids"].squeeze()
        attn_mask = output["attention_mask"].squeeze()
