...
from sandbox_ia.ml_model.autoencoders import AutoEncoder, AELoss
from sandbox_ia.sandbox_utils.logger import get_logger

logger = get_logger()
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

CHECKPOINTS_DIR = os.path.join(os.path.dirname(__file__), "checkpoints_ae")
...
class Trainer(nn.Module):
    ...
    def val_step(self, x, x_ebd, y):
        # Car pour l'ae, le y est le X, le X intial (ebd + seq)
        x = x.to(device) if hasattr(x, "to") else x
        x_ebd = x_ebd.to(device) if hasattr(x_ebd, "to") else x_ebd
        y = y.to(device) if hasattr(y, "to") else y
        y = y.float()
        logits = self.model(x_ebd, x)
        loss = self.loss(logits, y)
        return loss.item(), *self.model.predict(initial_x=y, x=x, x_ebd=x_ebd, logits=logits)
    ...
    def evaluate(self, valloader):
        ...
        with torch.no_grad():
            for batch in loop:
                ...
                loss, mse, mae = self.val_step(x, x_ebd, y)
                total_loss += loss
                num_batches += 1
                total_mse += mse
                total_mae += mae
...
def compute_reconstruction_errors(model, loader, device):
    """
    Calcule l'erreur de reconstruction (MSE) PAR ÉCHANTILLON, sur le modèle
    final déjà entraîné. À utiliser sur un loader de données "safe" (val/test)
    JAMAIS vues en entraînement, pour caractériser le comportement normal.

    Returns
    -------
    np.ndarray, shape (n_samples,)
        Une erreur MSE par séquence.
    """
    model.eval()
    errors = []
    with torch.no_grad():
        for batch in loader:
            batch = dict(batch)
            x = batch["x"].to(device)
            x_ebd = batch["x_ebd"].to(device)
            y = batch["y"].to(device).float()

            logits = model(x_ebd, x)
            # Moyenne sur (seq_len, features) -> 1 erreur par séquence, pas par epoch
            per_sample_mse = ((logits - y) ** 2).mean(dim=(1, 2))
            errors.extend(per_sample_mse.cpu().numpy().tolist())

    return np.array(errors)


def compute_threshold(errors: np.ndarray) -> dict:
    """
    Calcule plusieurs candidats de seuil à partir d'erreurs de reconstruction
    PAR ÉCHANTILLON (pas par epoch !). Recommandation : utiliser p99 comme
    seuil principal (peu de faux positifs), p95 en indicateur de vigilance.

    Parameters
    ----------
    errors : np.ndarray
        Erreurs de reconstruction par échantillon, calculées via
        compute_reconstruction_errors() sur du safe non vu en entraînement.
    """
    ...
    return {
        "mean": mean,
        "std": std,
        "mean_plus_2std": mean + 2 * std,   # référence, mais fragile si distribution skewed
        "p95": p95,
        "p99": p99,
        "recommended_threshold": p99,        # 👈 celui à utiliser en prod
        "min": errors.min(),
        "max": errors.max(),
