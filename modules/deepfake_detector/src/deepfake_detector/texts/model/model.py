...
class DeepFakeDetectorText(nn.Module):
    def __init__(
        self,
        d_model:int,
        num_heads:int,
    ...
    def build(
        ...
        self.ml_proba_head = nn.Linear(
            in_features=ml_proba_features,
            out_features=d_model,
            bias=True
        )
    ...
    def forward(self, embeddings:torch.Tensor, features:torch.Tensor|np.ndarray, ml_features:torch.Tensor|np.ndarray):
        if not isinstance(features, torch.Tensor):
            features = torch.tensor(features, device=DEVICE)
        
        if not isinstance(ml_features, torch.Tensor):
            ml_features = torch.tensor(ml_features, device=DEVICE)
        
        features = features.float()
        ml_features = ml_features.float()
        embeddings = embeddings.float()
        features:torch.Tensor = self.feature_head(features)
        ml_features:torch.Tensor = self.ml_proba_head(ml_features)
        # Embedding, shape 2d, [batch, d_model] ou 3d shape [batch, seq_len, d_model]
        # Features shape 2d, [batch, d_model]; ml_features, shape 2d, [batch, d_model]
        if self._params["cls_only"]:    
            if embeddings.ndim == 3:
                embeddings = embeddings[:, 0, :] # Prendre le premier en supposant cls token
            x:torch.Tensor = torch.stack([embeddings, features, ml_features], dim=1)  # Car transformer attention attend 3d
        else:
            if embeddings.ndim < 3:
            ...
            ml_features = ml_features.unsqueeze(1)
            x = torch.cat([embeddings, features, ml_features], dim=1)
        x = self.norm(x)
    ...
    def predict(
        ...
        ml_features:torch.Tensor|np.ndarray, 
        logits:torch.Tensor|None = None, 
        threashold:float = 0.5
    ):
        with torch.inference_mode():
            if logits is None:
                logits = self(embeddings=embeddings, features=features, ml_features=ml_features)
...
class DeepFakeDetectorTextDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if isinstance(ml_preds_path_or_df, pd.DataFrame):
            self.ml_pred_dataset = ml_preds_path_or_df
        
        else:
            self.text_dataset = pd.DataFrame(self.get_good_func(text_dataset_path_or_df))
            self.ml_pred_dataset = pd.DataFrame(self.get_good_func(ml_preds_path_or_df))
        
        assert "text" in self.text_dataset and "label" in self.text_dataset, "Certaines columns manquent"
        assert len(self.text_dataset) == len(self.ml_pred_dataset), "Les deux dataset doivent avoir la meme longueur"
    ...
    def __getitem__(self, index):
        ...
        features = torch.tensor(self.features_extractor(text))
        ml_features = torch.tensor(self.ml_pred_dataset.iloc[index], dtype=torch.float32)
