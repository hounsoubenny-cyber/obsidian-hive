...
# -*- coding: utf-8 -*-
"""
Created on Tue May  5 15:13:58 2026
...
class Trainer(nn.Module):
    def __init__(
        ...
        if compile_model:
            ...
            self.text_encoder = torch.compile(text_encoder)
        if compile_loss:
    ...
    def train_step(self, tokenizer_output, features, ml_features, y):
        embedding = self.text_encoder.predict(tokenizer_output)
        embedding = embedding.to(device) if hasattr(embedding, "to") else embedding
        features = features.to(device) if hasattr(features, "to") else features
        ml_features = ml_features.to(device) if hasattr(ml_features, "to") else ml_features
        ...
        logits = self.model(embedding, features, ml_features)
    ...
    def val_step(self, tokenizer_output, features, ml_features, y):
        embedding = self.text_encoder.predict(tokenizer_output)
        embedding = embedding.to(device) if hasattr(embedding, "to") else embedding
        features = features.to(device) if hasattr(features, "to") else features
        ml_features = ml_features.to(device) if hasattr(ml_features, "to") else ml_features
        ...
        logits = self.model(embedding, features, ml_features)
        loss = self.loss(logits, y)
        return loss.item(), self.model.predict(embedding, features, ml_features, logits, 0.5)[1].to(device)
    ...
    def train_on_epoch(self, dataloader):
        ...
        for batch in loop:
            ...
            ml_features = batch["ml_features"]
            batch.pop("label")
            batch.pop("features")
            batch.pop("ml_features")
            tokenizer_output = dict(batch)
            loss = self.train_step(tokenizer_output, features, ml_features, y)
    ...
    def evaluate(self, valloader):
        ...
        with torch.no_grad():
            for batch in loop:
                ...
                batch.pop("ml_features")
                tokenizer_output = dict(batch)
                loss, pred = self.val_step(tokenizer_output, features, ml_features, y)
                total_loss += loss
                num_batches += 1
                for metric in self.metrics.values():
                    metric(pred, y)
