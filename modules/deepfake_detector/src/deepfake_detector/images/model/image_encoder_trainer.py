...
class Trainer(nn.Module):
    ...
    def train_step(self, processor_output:dict, y:torch.Tensor):
        y = y.to(device) if hasattr(y, "to") else y
        y = y.long()
            
        self.optimizer.zero_grad()
        logits = self.model(processor_output, output2d=True)
    ...
    def val_step(self, processor_output, y):
        y = y.to(device) if hasattr(y, "to") else y
        y = y.long()
    ...
    def train_on_epoch(self, dataloader):
        ...
        for batch in loop:
            ...
            loss = self.train_step(processor_output, y)
    ...
    def evaluate(self, valloader):
        ...
        with torch.no_grad():
            for batch in loop:
                ...
                loss, embeddings = self.val_step(processor_output, y)
