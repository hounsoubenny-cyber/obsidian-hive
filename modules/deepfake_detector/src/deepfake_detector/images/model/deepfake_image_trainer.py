...
from deepfake_detector.images.model.image_encoder import ImageEncoder
from deepfake_detector.images.model.model import DeepFakeDetectorImage
...
class Trainer(nn.Module):
    ...
    def train_step(self, processor_output, features, y):
        embedding = self.image_encoder.predict(processor_output)
        ...
        logits = self.model(embedding, features)
    ...
    def val_step(self, processor_output, features, y):
        embedding = self.image_encoder.predict(processor_output)
        embedding = embedding.to(device) if hasattr(embedding, "to") else embedding
        features = features.to(device) if hasattr(features, "to") else features
        y = y.to(device) if hasattr(y, "to") else y
        ...
        logits = self.model(embedding, features)
        loss = self.loss(logits, y)
        return loss.item(), self.model.predict(embedding, features, logits, 0.5)[1].to(device)
    ...
    def train_on_epoch(self, dataloader):
        ...
        for batch in loop:
            ...
            batch.pop("label")
            batch.pop("features")
            processor_output = dict(batch)
            loss = self.train_step(processor_output, features, y)
    ...
    def evaluate(self, valloader):
        ...
        with torch.no_grad():
            for batch in loop:
                ...
                batch.pop("features")
                processor_output = dict(batch)
                loss, pred = self.val_step(processor_output, features, y)
                total_loss += loss
                num_batches += 1
                for metric in self.metrics.values():
                    metric(pred, y)
