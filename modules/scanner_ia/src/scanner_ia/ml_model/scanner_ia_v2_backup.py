...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 19 00:45:55 2026
...
class ScannerIA:
    ...
    def scanner_predict(self, X, threshold:float|np.ndarray = 0.5):
        ...
        for i, labels in list(to_return["predict"].items()):
            if safe_classe in labels:
                to_return["predict"][i] = [safe_classe]
                # proba_predict ne garde que les classes retenues dans predict ;
                # on utilise la VRAIE probabilité de SAFE (déjà calculée dans
                # "proba"), jamais une valeur aléatoire pour les autres classes.
                to_return["proba_predict"][i] = {safe_classe: to_return["proba"][i][safe_classe]}
