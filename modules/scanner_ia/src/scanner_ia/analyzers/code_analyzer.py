                ...
                for sig in signatures:
                    ...
                    for pat in pattern:
                        for m in pat.finditer(text):
                            ...
                            # Le check anti-faux-positif doit porter sur la VALEUR capturée
                            # (ex: le mot de passe lui-même), pas sur le match complet — sinon
                            # un pattern comme "db_password = ..." s'auto-annule : le nom du
                            # champ "db_password" contient déjà le mot-clé "password" utilisé
                            # pour filtrer les faux positifs, donc TOUT match (y compris un
                            # vrai secret) finissait ignoré. Repli sur le match complet si le
                            # pattern n'a pas de groupe capturant (ex: clé AWS, déjà "nue").
                            fp_check_value = m.groups()[-1] if m.groups() else find
                            if self._is_false_positive(fp_check_value, sig.get('false_positive_keywords', [])):
